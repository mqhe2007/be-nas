...
- 规划媒体目录：电影、剧集、动画、纪录片、音乐、字幕、转码缓存和元数据缓存。推荐结构 `/data/media/影视/电影/` 和 `/data/media/影视/电视/`，Jellyfin 各建一个媒体库分别指向。
...
- **代理策略**：不是所有服务都需要代理，按服务实际需要判断。国内 PT 站直连可访问时，qBittorrent 无需代理；Jellyfin/Prowlarr/Radarr/Sonarr 等需要访问 TMDB/索引器定义的才配置代理。详见 `references/docker-proxy-pattern.md` 的「代理策略：按服务实际需要决定」章节。
- qBittorrent 部署具体配置（Compose、分类、代理判断）见 `references/qbittorrent-deployment.md`。
- **\*arr 全家桶部署**：qBittorrent + Prowlarr + Radarr + Sonarr 的完整部署与串联配置，包括网络规划、代理策略、硬链接验证，见 `references/arr-stack-deployment.md`。
...
- qBittorrent 部署注意事项（镜像选择、代理配置、分类目录、硬链接前提）见 `references/qbittorrent-deployment.md`。
...
- 云盘备份首选用户自开发的 `baidupan-cli`（详见 `references/baidupan-cli.md`）：原始目录结构直传 + SQLite 状态追踪增量 + 本地加密 + 并行上传，支持百万级文件；恢复时直接 download 即可复原。备份脚本位于 `~/.hermes/scripts/baidupan-backup.py`。
- **排除策略**：公共资源刮削产物不备份——Jellyfin 元数据、ML 缓存、插件解包文件等可通过服务重扫/重建恢复的数据一律排除，只备份用户的真实数据和服务核心配置（数据库、配置文件、种子、用户设置等）。具体排除列表见 `references/baidupan-cli.md`。
- **备份策略**：
  - 全量首次 + 增量日常（不打包，保持原目录结构）
  - 增量通过 mtime+size 比对，SQLite 记录状态
  - 本地删除 → 云端保留（墓碑机制，不自动清理）
  - 本地移动 → 远程 `baidupan-cli mv`（size 匹配检测，避免重传）
  - 本地修改 → 重新加密上传覆盖
